  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.fulfilled, async (state, action) => {
        ...
        // Salvar dados do usuário com o token
        await storage.setUserData({
          user: action.payload.user,
          token: action.payload.token,
          offlineMode: action.payload.offlineMode || false
        });
      })
      ...
      .addCase(checkAuth.fulfilled, async (state, action) => {
        state.loading = false;
        if (action.payload) {
          state.isAuthenticated = true;
          state.user = action.payload.user;
      ...
      .addCase(logoutUser.fulfilled, async (state, action) => {
        state.loading = false;
        state.isAuthenticated = false;
        state.user = null;
        state.token = null;
